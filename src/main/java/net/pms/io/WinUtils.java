...
public class WinUtils extends BasicSystemUtils {
	...
	protected String findPsPing() {
		LOGGER.debug("Looking for psping..");
		try {
				...
				if (psPingProcess.getOutput() != null && !psPingProcess.getOutput().isEmpty()) {
					for (String line : psPingProcess.getOutput()) {
						if (isNotBlank(line) && line.startsWith("PsPing")) {
							LOGGER.debug("Found psping64.exe");
							return "psping64.exe";
						}
					}
				}
