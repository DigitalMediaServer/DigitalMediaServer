 ...
 */
public class FileNamePrettifier { //TODO: (Nad) Replace with this
	...
	/**
	 * Capitalize the first letter of each word if the name is in all in the
	 * same case.
	 *
	 * @param name the name to process.
	...
	protected String convertFormattedNameToTitleCase(String name) {
		...
		if (StringUtil.isSameCase(name)) {
			return TitleCase.convert(name, locale);
		}
		return name;
	}
