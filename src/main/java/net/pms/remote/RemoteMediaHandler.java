	...
	public void handle(HttpExchange httpExchange) throws IOException {
		try {
			...
			if (resource.getFormat().isVideo()) {
				if (flash) {
					...
					if (FileUtil.isUrl(resource.getSystemName())) {
						resource.setPlayer(PlayerFactory.getPlayer(PlayerId.FFMPEG_WEB_VIDEO));
					} else if (!(resource instanceof DVDISOTitle)) {
						resource.setPlayer(PlayerFactory.getPlayer(PlayerId.FFMPEG_VIDEO));
					}
					//code = 206;
				}
				if (
			...
			if (!RemoteUtil.directmime(mimeType) && resource.getFormat().isAudio()) {
				resource.setPlayer(PlayerFactory.getPlayer(PlayerId.FFMPEG_AUDIO));
				code = 206;
			}
