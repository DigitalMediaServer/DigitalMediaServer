...
public class ImagesUtil {
	...
	private static final HashMap<ISO639, WeakReference<BufferedImage>> LANGUAGE_FLAGS_CACHE = new HashMap<>();
	...
	protected static Image transcodeImage(
		...
		Image result;
		if (dlnaThumbnail) {
			result = new DLNABinaryThumbnail(
				outputByteArray,
				bufferedImage.getWidth(),
				bufferedImage.getHeight(),
				outputFormat,
	 ...
	 * more than once.
	 *
	 * @param language the {@link ISO639} language whose "flag" to
	 *            get.
	 * @return The {@link BufferedImage} with the "flag" representing this
	 *         language code or {@code null}.
	 */
	@Nullable
	public static BufferedImage getLanguageFlag(@Nullable ISO639 language) {
		if (language == null) {
			return null;
		}
		...
		synchronized (LANGUAGE_FLAGS_CACHE) {
			...
			Iterator<Entry<ISO639, WeakReference<BufferedImage>>> cacheIterator = LANGUAGE_FLAGS_CACHE.entrySet().iterator();
			while (cacheIterator.hasNext()) {
				Entry<ISO639, WeakReference<BufferedImage>> entry = cacheIterator.next();
				if (entry.getValue() != null && entry.getValue().get() == null) {
					cacheIterator.remove();
				}
			}

			if (LANGUAGE_FLAGS_CACHE.containsKey(language)) {
				WeakReference<BufferedImage> reference = LANGUAGE_FLAGS_CACHE.get(language);
				if (reference == null) {
				...
				LANGUAGE_FLAGS_CACHE.remove(language);
			}

			InputStream inputStream = PMS.class.getResourceAsStream(String.format(LANGUAGE_FLAGS_PATH, language.getCode()));
			if (inputStream != null) {
				try {
					BufferedImage result = ImageIO.read(inputStream);
					LANGUAGE_FLAGS_CACHE.put(language, new WeakReference<BufferedImage>(result));
					return result;
				} catch (Exception e) {
					...
					LOGGER.warn(
						...
						language,
						String.format(LANGUAGE_FLAGS_PATH, language.getCode()),
						e.getMessage()
					);
					LOGGER.trace("", e);
				}
			...
			LANGUAGE_FLAGS_CACHE.put(language, null);
			return null;
		}
	}
	...
	public static class AudioFlagFilter extends NonGeometricBufferedImageOp implements BufferedImageFilter {
		 ...
		 *
		 * @param language the {@link ISO639} for the language.
		 * @param hints the {@link RenderingHints} or {@code null}.
		 */
		public AudioFlagFilter(ISO639 language, RenderingHints hints) {
			super(hints);
		...
		public String toString() {
			return "Audio language flag for " + language;
		}

		/**
		 * @return The {@link ISO639} specified in the constructor.
		 */
		...
		public BufferedImageFilterResult filter(BufferedImage source, BufferedImage destination, boolean modifySource) {
			BufferedImage flag = getLanguageFlag(language);
			if (flag == null) {
	...
	public static class SubtitlesFlagFilter extends NonGeometricBufferedImageOp implements BufferedImageFilter {
		 ...
		 * @param hints the {@link RenderingHints} or {@code null}.
		 */
		public SubtitlesFlagFilter(ISO639 language, RenderingHints hints) {
			super(hints);
			this.language = language;
		}
		...
		@Override
		public String toString() {
			return "Subtitles language flag for " + language;
		}
