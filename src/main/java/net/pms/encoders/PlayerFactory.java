...
public final class PlayerFactory {
	...
	private static boolean playerTest(Player player, Path executable) throws InterruptedException {
		...
		if (player instanceof FFMpegVideo || player instanceof FFmpegDVRMSRemux) {
			...
			TEST_RECORDS.add(new PlayerTestRecord(executable, player.isAvailable(), state));
			return true;
			...
			try {
				...
				if (result.getExitCode() == 0) {
					...
					state = String.format(Messages.getString("Engine.ExitCode"), player, result.getExitCode());
			...
			try {
				...
				if (result.getOutput() != null && StringUtils.isBlank(result.getOutput().get(0))) {
