...
import net.pms.configuration.FFmpegExecutableInfo.ProtocolOptions;
	...
	public boolean isCompatible(DLNAResource resource) {
		if (PlayerUtil.isWebVideo(resource)) {
			...
			if (executableInfo instanceof FFmpegExecutableInfo) {
				EnumSet<ProtocolOptions> options = ((FFmpegExecutableInfo) executableInfo).getProtocols().get(url.split(":")[0]);
				if (options == null || !options.contains(ProtocolOptions.INPUT)) {
					return false;
				}
			} else {
				LOGGER.warn(
