			...
			if (cueSheet != null) {
				...
				for (FileData fileData : cueSheet.getFileData()) {
					...
					for (int i = 0; i < tracks.size(); i++) {
						...
						if (trackMedia != null && cueSheetEntry.isPartialSource()) {
							if (cueSheetEntry.getClipEnd() == 0.0 || Double.isInfinite(cueSheetEntry.getClipEnd())) {
								Double originalDuration = trackMedia.getDuration();
								if (originalDuration != null) {
									trackMedia.setDuration(originalDuration.doubleValue() - cueSheetEntry.getClipStart());
								}
							} else {
								trackMedia.setDuration(cueSheetEntry.getClipEnd() - cueSheetEntry.getClipStart());
							}
						}
